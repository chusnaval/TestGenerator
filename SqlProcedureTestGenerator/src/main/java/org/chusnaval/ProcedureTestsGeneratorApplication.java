...
public class ProcedureTestsGeneratorApplication implements CommandLineRunner {
	...
	private static final Logger log = LoggerFactory.getLogger(ProcedureTestsGeneratorApplication.class);

	public static void main(String[] args) {

		new SpringApplicationBuilder(ProcedureTestsGeneratorApplication.class).web(WebApplicationType.NONE).run(args);
	}
	...
	public void run(String... args) {
		...
		try {
			...
			if (isValidInputsInCommandLine(line)) {
				SqlTestGenerator generator = new SqlTestGenerator(line.hasOption(RECURSIVE), new FileFinder("glob:*.java"));
